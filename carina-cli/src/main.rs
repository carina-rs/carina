...
async fn run_plan(path: &PathBuf, out: Option<&PathBuf>) -> Result<bool, String> {
    ...
    if will_create_state_bucket {
        ...
        println!(
            ...
            "+".green(),
            format!("aws.s3_bucket.{}", state_bucket_name).green()
        );
        println!(
        ...
        if !bucket_exists {
            ...
            if let Some(bucket_resource) = find_state_bucket_resource(&parsed, &bucket_name) {
                ...
                if auto_create {
                    ...
                    let bucket_state = ResourceState::new("s3_bucket", &bucket_name, "aws")
...
async fn run_destroy(path: &PathBuf, auto_approve: bool) -> Result<(), String> {
    ...
    let resources_to_destroy: Vec<&Resource> = destroy_order
        ...
        .filter(|r| {
            ...
            if r.id.resource_type == "s3_bucket"
...
/// - provider.resource.TypeName.value (e.g., aws.s3_bucket.VersioningStatus.Enabled, awscc.ec2_vpc.InstanceTenancy.default)
...
async fn run_state_bucket_delete(
    ...
    let bucket_id = ResourceId::with_provider("aws", "s3_bucket", bucket_name);
    match aws_provider
...
mod tests {
    ...
    fn plan_file_serde_round_trip() {
        ...
        plan.add(Effect::Create(
            Resource::with_provider("aws", "s3_bucket", "my-bucket")
                .with_attribute("name", Value::String("my-bucket".to_string())),
        ));
        plan.add(Effect::Delete {
            id: ResourceId::with_provider("aws", "s3_bucket", "old-bucket"),
        ...
        let current_states = vec![CurrentStateEntry {
            id: ResourceId::with_provider("aws", "s3_bucket", "my-bucket"),
            state: State::not_found(ResourceId::with_provider("aws", "s3_bucket", "my-bucket")),
        }];
    ...
    fn test_detailed_exitcode_with_changes() {
        ...
        let mut plan = Plan::new();
        plan.add(Effect::Create(Resource::new("s3_bucket", "test")));
        let has_changes = !plan.is_empty();
        assert!(has_changes);
    }
}
