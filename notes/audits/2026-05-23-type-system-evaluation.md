...
例外は `Custom` の経路:
- `Custom→Custom`: `TypeIdentity::same_type` の per-axis **同値** +
  `pattern` 文字列等価 + `length` 区間包含 (`narrow ⊆ wide`) +
  `base` への再帰。`pattern`/`length` は片方向 subsumption だが
  `TypeIdentity` 部分は対称な同値関係になっている (§3.3 (n) 参照、
  carina#3218 で soundness バグとして起票)。
...
**(c) `TypeIdentity` の三軸構造そのものは良い**
- `provider + segments + kind` の三軸に同一性を分解する設計は、
  **provenance を運ぶ nominal type** として方向性は正しい。
- `aws.iam.Role.Arn` と `aws.acm.Certificate.Arn` を区別し、
  `aws.Region` と `gcp.Region` を区別する — どちらも multi-provider IaC
  で必要な distinctions。
- **ただし実装の `same_type` は同値関係になっていて、本来の片方向
  subsumption (`具体 ≤ 抽象`) が壊れている**。§3.3 (n) に詳述。
  「軸の設計は良いが、軸を使う比較関数がバグっている」という構図。
...
**(n) `TypeIdentity::same_type` が同値関係になっている (soundness バグ)**
- `type_identity.rs:133` の `same_type` は per-axis で「両方
  populated のときだけ一致必須、片方が空なら通す」という規則。
  これは**対称な同値関係**で書かれている。
- ところが `is_assignable_to` の Custom→Custom アーム
  (`schema/mod.rs:1404-1414`) はこれを片方向 subsumption の判定として
  使っている。同じアーム内の `pattern` (片方向: sink Some / source
  None で拒否) と `length` (区間包含 `source ⊆ sink`) は方向性のある
  規則で書かれているのに、`identity` だけが対称になっている。
- 結果として、本来拒否すべき**抽象 → 具体**の代入が通る。例:

  ```rust
  // 実テスト (schema/tests.rs:2742)
  let generic = mk(&[]);                  // aws.Arn
  let role_arn = mk(&["iam", "Role"]);    // aws.iam.Role.Arn
  assert!(generic.is_assignable_to(&role_arn));   // 通ってしまう ✗
  assert!(role_arn.is_assignable_to(&generic));   // 通る (これは OK)
  ```

  `aws.Arn` 型の値(出元不明の汎用 ARN — `aws.acm.Certificate.Arn`
  かもしれない)を、`aws.iam.Role.Arn` を受け取る attribute に渡せて
  しまう。**provider 軸でクラウド跨ぎを防ぐ**という設計目標を、
  segments 軸では実現できていない。
- 同じ `is_assignable_to` 内に「`Custom { identity: None }` source →
  `Custom { identity: Some(_) }` sink は拒否」(`:1417`) という規則が
  あり、コメントも「source has no proof of identity」と明記。**この
  精神を `TypeIdentity` 内部の軸にも適用すれば直る** — carina#3218
  で起票。
- §3.3 (i) の subtyping lattice property test (反対称性) が入っていれば
  機械的に検出できた具体例。Issue #3208 とセットで対応すれば回帰
  ネットになる。
...
**B 〜 B+**(soundness バグ §3.3 (n) が直れば B+〜A-)。

- **長所**: refinement types + 三軸 nominal identity の**設計**は、
  IaC ドメインに対して 2026 年時点でほぼ最適解に近い。
  sound-over-complete の姿勢も明確。
- **短所**: 形式化が弱い (subtyping lattice 公理化なし)、二重実装、
  `type_name()` 文字列化、variance 暗黙 — 実装が先行して理論が
  追いついていない状態。さらに `TypeIdentity::same_type` の対称性
  バグ (§3.3 (n)) のように、**設計意図と実装挙動がズレている箇所**が
  property test の欠如によって検出できていない。
...
0. **(n) `TypeIdentity::same_type` の対称性 soundness バグ修正**
   (carina#3218) — `same_type` (対称) と
   `assignable_to(source, sink)` (片方向) を分離。具体 → 抽象は通し、
   抽象 → 具体は拒否する。最優先。
1. **(i) Subtyping lattice の公理化と性質テスト** — reflexivity /
   transitivity / antisymmetry を property test で担保。(n) を機械的に
   検出するネットでもある。
