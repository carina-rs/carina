...
- **Codegen** (`carina-provider-aws/carina-codegen-aws/src/resource_defs.rs`): two parallel structs `ResourceDef` and `DataSourceDef`, each in its own `Vec`. Their fields differ substantially (`ResourceDef` has `create_op`, `update_ops`, `delete_op`, `force_replace`, `unique_name_attribute`, etc.; `DataSourceDef` has `inputs: Vec<DataSourceInput>` and `read_ops` only). No type currently appears in both lists.
