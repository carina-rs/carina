...
: create-only diff から直接 `Effect::Replace` を作らず、まず `PendingReplace` として保持する。dependent scan 後に CBD/DBC、temporary name、cascade reason を確定し、`decompose_replace_into_effects` で Create/Update/Delete と display metadata を生成する。
...
Stage 2 では `PendingReplace` に `create_before_destroy: bool` を持たせる。dependent scan は `Plan` 上の Replace を mutate せず、PendingReplace の CBD/DBC フラグを確定する。plan に effects を追加するのは分解確定後だけにする。
...
`PendingReplace` は `id/from/to/directives/changed_create_only/temporary_name/cascade_ref_hints/create_before_destroy` を持つ。dependent scan は `PendingReplace` を更新し、CBD/DBC を確定する。最後に `decompose_replace_into_effects` が Create、consumer Update、old Delete、必要なら rename Update、`ReplaceDisplayMetadata` を生成する。
...
unit では `decompose_replace_into_effects` を直接テストする。DBC、CBD、consumer update 追加、既存 consumer update 再利用、temporary name、rename Update、prevent_destroy error を分ける。
