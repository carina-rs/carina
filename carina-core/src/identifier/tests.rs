...
fn make_awscc_provider_with_region(region: &str) -> ProviderConfig {
    ProviderConfig {
        name: "awscc".to_string(),
        attributes: vec![(
            "region".to_string(),
            Value::Concrete(ConcreteValue::EnumIdentifier(region.to_string())),
        )]
        .into_iter()
        .collect(),
...
fn anonymous_hash_suffix(identifier: &str) -> &str {
    identifier
        .rsplit('_')
        .next()
        .expect("anonymous identifier should end with a hash suffix")
}

fn route_schema_without_create_only_attributes() -> ResourceSchema {
...
fn eip_schema_with_domain_enum() -> ResourceSchema {
    ResourceSchema::new("ec2.Eip").attribute(AttributeSchema::new(
        "domain",
        AttributeType::enum_(
            crate::schema::enum_identity("Domain", Some("awscc.ec2.Eip")),
...
fn anonymous_route_resource() -> Resource {
    let mut r = Resource::with_provider("awscc", "ec2.Route", "", None);
    r.set_attr(
        "route_table_id".to_string(),
        Value::Concrete(ConcreteValue::String("rtb-0bdcde53c2f569e58".to_string())),
    );
    r.set_attr(
    ...
    r.set_attr(
        ...
        Value::Concrete(ConcreteValue::String("igw-0e6fc44cdbd73a537".to_string())),
    );
    r
}
...
fn test_anonymous_id_stable_across_provider_namespace_change_in_identity() {
    let mut schemas = SchemaRegistry::new();
    schemas.insert("awscc", route_schema_without_create_only_attributes());
    let identity_fn = |_: &str| -> Vec<String> { vec!["region".to_string()] };

    let old_providers = vec![make_awscc_provider_with_region(
        "awscc.Region.ap_northeast_1",
    )];
    let new_providers = vec![make_awscc_provider_with_region("aws.Region.ap_northeast_1")];

    let mut old_resources = vec![anonymous_route_resource()];
    let mut new_resources = vec![anonymous_route_resource()];

    compute_anonymous_identifiers(&mut old_resources, &old_providers, &schemas, &identity_fn)
        .unwrap();
    compute_anonymous_identifiers(&mut new_resources, &new_providers, &schemas, &identity_fn)
        .unwrap();

    assert_eq!(
        anonymous_hash_suffix(old_resources[0].id.name_str()),
        anonymous_hash_suffix(new_resources[0].id.name_str()),
        "provider namespace prefixes in identity enum values should not change the anonymous hash",
    );
}
...
fn test_anonymous_id_stable_across_provider_namespace_change_in_attribute() {
    let mut schemas = SchemaRegistry::new();
    schemas.insert("awscc", eip_schema_with_domain_enum());
    let providers = vec![ProviderConfig {
        name: "awscc".to_string(),
        attributes: vec![(
            "region".to_string(),
    ...
    let mut old_resources = vec![make_eip("awscc.ec2.Eip.Domain.vpc")];
    let mut new_resources = vec![make_eip("aws.ec2.Eip.Domain.vpc")];

    compute_anonymous_identifiers(&mut old_resources, &providers, &schemas, &identity_fn).unwrap();
    compute_anonymous_identifiers(&mut new_resources, &providers, &schemas, &identity_fn).unwrap();

    assert_eq!(
        anonymous_hash_suffix(old_resources[0].id.name_str()),
        anonymous_hash_suffix(new_resources[0].id.name_str()),
        "provider namespace prefixes in resource enum attributes should not change the anonymous hash",
    );
}
...
fn test_reconcile_anonymous_id_after_provider_namespace_change() {
    ...
    reconcile_anonymous_identifiers(
        &mut new_resources,
        &schemas,
        &|_provider, _resource_type| state_entries.clone(),
    );

    assert_eq!(
        new_resources[0].id.name_str(),
        old_name,
        "after canonicalizing enum hash inputs, the namespace-only provider change should rebind to the state identifier",
    );
}
