...
pub fn get_enum_valid_values(
    ...
    match (resource_type, attr_name) {
        ...
        ("ec2_transit_gateway", "security_group_referencing_support") => Some(&["enable", "disable"]),
...
fn validate_service_arn(arn: &str, expected_service: &str, resource_prefix: Option<&str>) -> Result<(), String> {
    validate_arn(arn)?;
    let parts: Vec<&str> = arn.splitn(6, ':').collect();
    if parts[2] != expected_service {
        return Err(format!("Expected {} ARN, got service '{}'", expected_service, parts[2]));
    }
    if let Some(prefix) = resource_prefix
        && !parts[5].starts_with(prefix)
    {
        return Err(format!("Expected resource starting with '{}', got '{}'", prefix, parts[5]));
    }
    Ok(())
}
...
mod tests {
    ...
    fn validate_iam_role_arn_valid() {
        let t = iam_role_arn();
        assert!(t.validate(&Value::String("arn:aws:iam::123456789012:role/MyRole".to_string())).is_ok());
        assert!(t.validate(&Value::String("arn:aws:iam::123456789012:role/path/to/MyRole".to_string())).is_ok());
        // ResourceRef should be accepted
        assert!(t.validate(&Value::ResourceRef("role".to_string(), "arn".to_string())).is_ok());
    }
    ...
    fn validate_iam_role_arn_invalid() {
        let t = iam_role_arn();
        // Wrong service
        assert!(t.validate(&Value::String("arn:aws:s3:::my-bucket".to_string())).is_err());
        // Wrong resource prefix
        assert!(t.validate(&Value::String("arn:aws:iam::123456789012:policy/MyPolicy".to_string())).is_err());
        // Not an ARN at all
        assert!(t.validate(&Value::String("not-an-arn".to_string())).is_err());
    }

    #[test]
    fn validate_iam_policy_arn_valid() {
        let t = iam_policy_arn();
        assert!(t.validate(&Value::String("arn:aws:iam::123456789012:policy/MyPolicy".to_string())).is_ok());
        assert!(t.validate(&Value::String("arn:aws:iam::aws:policy/AdministratorAccess".to_string())).is_ok());
        // ResourceRef should be accepted
        assert!(t.validate(&Value::ResourceRef("policy".to_string(), "arn".to_string())).is_ok());
    }
    ...
    fn validate_iam_policy_arn_invalid() {
        let t = iam_policy_arn();
        // Wrong resource prefix
        assert!(t.validate(&Value::String("arn:aws:iam::123456789012:role/MyRole".to_string())).is_err());
        // Wrong service
        assert!(t.validate(&Value::String("arn:aws:s3:::my-bucket".to_string())).is_err());
    }
    ...
    fn validate_kms_key_arn_valid() {
        let t = kms_key_arn();
        assert!(t.validate(&Value::String("arn:aws:kms:us-east-1:123456789012:key/1234abcd-12ab-34cd-56ef-1234567890ab".to_string())).is_ok());
        // ResourceRef should be accepted
        assert!(t.validate(&Value::ResourceRef("key".to_string(), "arn".to_string())).is_ok());
    }
    ...
    fn validate_kms_key_arn_invalid() {
        ...
        assert!(t.validate(&Value::String("arn:aws:iam::123456789012:role/MyRole".to_string())).is_err());
        // Wrong resource prefix
        assert!(t.validate(&Value::String("arn:aws:kms:us-east-1:123456789012:alias/my-key".to_string())).is_err());
    }
