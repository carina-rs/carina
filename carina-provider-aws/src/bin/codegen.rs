...
//!
//! This tool generates Rust schema code for carina-provider-aws
...
//!     aws-codegen --type-name AWS::EC2::VPC
//!
//!   # Generate from file
//!   aws-codegen --file schema.json --type-name AWS::EC2::VPC
...
#[derive(Parser, Debug)]
#[command(name = "aws-codegen")]
#[command(about = "Generate Carina AWS provider schema code from CloudFormation schemas")]
struct Args {
...
fn generate_markdown(schema: &CfnSchema, type_name: &str) -> Result<String> {
    ...
    let namespace = format!("aws.{}", full_resource);
...
fn generate_schema_code(schema: &CfnSchema, type_name: &str) -> Result<String> {
    ...
    // Always need AttributeType for the injected `name` attribute (AttributeType::String)
    let mut needs_attribute_type = true;
...
"#,
    ...
    let schema_name = format!("aws.{}", full_resource);
...
"#,
    ...
    // Close the schema (ResourceSchema) and the AwsSchemaConfig struct
    code.push_str("    }\n}\n");
